...
describe('exposure maps key to the SAME lens lists', () => {
  it('country, state, and industry exposures cover every id exactly once', () => {
    expect(GEO_EXPOSURE_COUNTRIES.map((g) => g.id).sort()).toEqual(REPORT_COUNTRIES.map((c) => c.id).sort());
...
describe('the military layer', () => {
  it('covers all eleven countries with budget, %GDP, alignment, and nuclear status', () => {
    expect(MILITARY_BALANCE.map((m) => m.id).sort()).toEqual(REPORT_COUNTRIES.map((c) => c.id).sort());
    for (const m of MILITARY_BALANCE) {
      expect(m.budgetB).toBeGreaterThan(0);
      expect(m.pctGdp).toBeGreaterThan(0.5);
      expect(m.pctGdp).toBeLessThan(5);
      expect(m.note.length).toBeGreaterThan(50);
    }
    const by = (id: string) => MILITARY_BALANCE.find((m) => m.id === id)!;
    expect(by('us').budgetB).toBe(Math.max(...MILITARY_BALANCE.map((m) => m.budgetB)));
    ...
    // nuclear roster among the eleven: US, China, UK, France, India
    expect(MILITARY_BALANCE.filter((m) => m.nuclear).map((m) => m.id).sort()).toEqual(['china', 'france', 'india', 'uk', 'us']);
