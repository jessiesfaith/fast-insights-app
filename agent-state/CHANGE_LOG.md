...
- **User-guide pane + on-page skill text (same day, user request):** sticky right-pane "User guide" on every tab (what it's for · how to use it · the formulas that matter · a worked example, incl. softmax/z-score, precision/recall, INDEX/MATCH, DAX, cron-field anatomy); tab 8 now embeds the FULL text of the agent runbook (`AGENT_RUNBOOK_TEXT`) next to the already-embedded prompt; layout widened to 1400px two-column (`.fml-layout`, collapses <1040px).
- **What-if sandbox (same day, user request):** exact model v1.0 re-implemented in TS (`MODEL_PARAMS` + `scoreCompany` in `src/lib/financialModel.ts`, scaler + weights extracted from the pickle; tests prove it reproduces `predict_proba` on all 12 rows) powering a live sandbox at the end of tab 4 — pick a company, drag the 7 features, watch probabilities/recommendation/close-call flag + per-feature logit contributions update; 3 verified guided experiments.
- **Agent layer (same day, user request):** `financial-model/agent/AGENT_RUNBOOK.md` (permissions matrix + verify-first procedure, written as a control doc — doubles as "what an agent skill is") + `agent/memo_prompt.md` (10-minute exercise: run Claude as the analysis agent over the outputs, stopping at human approval); two new sections in tab 8; kit zip + public downloads refreshed. Commit `99fe45a`.
- **Deploy:** merged to `main` (`99fe45a`) with Jessica's approval and pushed 2026-08-28 → Vercel auto-deploy. Live URL not verifiable from the remote session (egress proxy 403s the domain) — confirm by opening app.fastinsights.io/financial-model. If the kit changes, re-run `python run_pipeline.py`, refresh the baked constants in `src/lib/financialModel.ts`, and re-zip `public/financial-model/financial-model-kit.zip`.
